...
public class TokenValidationAspect {
  ...
  /**
   * Advice that makes sure that before every method call that calls Bungie's API has a valid
   * access_token, if it does not, then refresh it.
   */
          ...
          if (c.statusCode().is2xxSuccessful()) {
            ...
            return c.createException()
                .flatMap(err -> Mono.error(
                    new ResourceNotFoundException(err.getResponseBodyAsString())));
          }
        }).block();
